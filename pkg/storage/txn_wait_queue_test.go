...
func createTxnForPushQueue(ctx context.Context, tc *testContext) (*roachpb.Transaction, error) {
	txn := newTransaction("txn", roachpb.Key("a"), 1, isolation.SERIALIZABLE, tc.Clock())
...
func TestTxnWaitQueueEnableDisable(t *testing.T) {
	...
	pusher := newTransaction("pusher", roachpb.Key("a"), 1, isolation.SERIALIZABLE, tc.Clock())
...
func TestTxnWaitQueueUpdateTxn(t *testing.T) {
	...
	pusher1 := newTransaction("pusher1", roachpb.Key("a"), 1, isolation.SERIALIZABLE, tc.Clock())
	pusher2 := newTransaction("pusher2", roachpb.Key("a"), 1, isolation.SERIALIZABLE, tc.Clock())
...
func TestTxnWaitQueuePusheeExpires(t *testing.T) {
	...
	txn := newTransaction("txn", roachpb.Key("a"), 1, isolation.SERIALIZABLE, clock)
...
func TestTxnWaitQueueDependencyCycleWithPriorityInversion(t *testing.T) {
	...
	txnA := newTransaction("txn", roachpb.Key("a"), -1, isolation.SERIALIZABLE, tc.Clock())
	...
	txnB := newTransaction("txn", roachpb.Key("a"), -2, isolation.SERIALIZABLE, tc.Clock())
