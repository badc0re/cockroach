...
func TestGCQueueMakeGCScoreRealistic(t *testing.T) {
...

	{
		...
		ms, valSize := initialMS(), 1<<10
		txn := newTransaction(
			"txn", roachpb.Key("key"), roachpb.NormalUserPriority, isolation.SERIALIZABLE,
...
func TestGCQueueProcess(t *testing.T) {
	...
	for i, datum := range data {
		if datum.del {
			...
			if datum.txn {
				txn = newTransaction("test", datum.key, 1, isolation.SERIALIZABLE, tc.Clock())
...
func TestGCQueueTransactionTable(t *testing.T) {
	...
	for strKey, test := range testCases {
		...
		txn := newTransaction("txn1", baseKey, 1, isolation.SERIALIZABLE, txnClock)
...
func TestGCQueueIntentResolution(t *testing.T) {
	...
	txns := []*roachpb.Transaction{
		newTransaction("txn1", roachpb.Key("0-0"), 1, isolation.SERIALIZABLE, tc.Clock()),
		newTransaction("txn2", roachpb.Key("1-0"), 1, isolation.SERIALIZABLE, tc.Clock()),
	}
